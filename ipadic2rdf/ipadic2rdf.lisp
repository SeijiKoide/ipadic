...
(defstruct (品詞 (:type list) :named (:conc-name hinshi-))
  data)

(defstruct (品詞-data (:type list) (:conc-name hinshi-data-))
...
(defstruct (語 (:type list) (:conc-name word-))
  lemma reading pronunciation)

(defstruct (見出し語 (:type list) :named (:conc-name lemma-))
  data)

(defstruct (word-lemma-data (:type list))
  name cost)

(defstruct (読み (:type list) :named (:conc-name yomi-))
  data)

(defstruct (発音 (:type list) :named (:conc-name hatuon-))
  data)
...
(defun ipadic2rdf ()
             ...
             (loop for line = (read-line ipastream nil nil)
                 ...
                 do (multiple-value-bind (hinshi pos) (read-from-string line nil nil)
                      (let ((word (read-from-string line nil nil :start pos)))
                        (assert (string= "名詞" (hinshi-type hinshi)))
  ...
  (let ((rdffile (make-pathname :name "ipadic" :type "rdf" :defaults *ipadic-directory*)))
